...
function displayICCID() {
    document.getElementById("iccid").innerHTML = `Generated ICCID: ${generateICCID()}`;
}
...
function generateMultipleEIDs() {
    let count = document.getElementById('eid-count').value;
    let eids = [];
    if (count > 0 && count <= 10) {
        for (let i = 0; i < count; i++) {
...
function displayIMEI() {
    document.getElementById("imei").innerHTML = `Generated IMEI: ${generateIMEI()}`;
}
